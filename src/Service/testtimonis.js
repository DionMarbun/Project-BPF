...
const API_URL = "https://gupajcdnaawdxmlkewxq.supabase.co/rest/v1/testimonis"
...
export const testimonis = {
    ...
    async createTestimonis(data) {
        const response = await axios.post(API_URL, data, { headers })
        return response.data
    },

    // Update status atau isi pesan (optional)
    async updateTestimonis(id, data) {
        const response = await axios.patch(`${API_URL}?id=eq.${id}`, data, { headers })
        return response.data
    },

    // Hapus data berdasarkan ID
    async deleteTestimonis(id) {
